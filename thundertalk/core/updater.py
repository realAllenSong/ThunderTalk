...
def install_update(zip_path: pathlib.Path, app_path: pathlib.Path) -> None:
    ...
    helper.write_text(
        ...
        "# overwrite the bundle. Bound the wait at 30 s; if the user\n"
        "# refused to quit, abort cleanly.\n"
        "for i in {1..60}; do\n"
        ...
        "BAK=\"${OLD}.update-old.$$\"\n"
        "mv \"$OLD\" \"$BAK\" || exit 1\n"
        "cp -R \"$NEW\" \"$OLD\" || { mv \"$BAK\" \"$OLD\"; exit 1; }\n"
        "xattr -dr com.apple.quarantine \"$OLD\" 2>/dev/null || true\n"
        "rm -rf \"$BAK\"\n"
        "open \"$OLD\"\n"
        "# Self-cleanup: remove the temp extraction dir after relaunch.\n"
        "sleep 2\n"
